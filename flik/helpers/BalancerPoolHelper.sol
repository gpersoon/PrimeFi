...
contract BalancerPoolHelper is IBalancerPoolHelper, Initializable, OwnableUpgradeable, BNum, DustRefunder {
	...
	/// @notice Balancer pool factory address
	IWeightedPoolFactory public poolFactory;

	/// @notice PRFI weight
	uint256 public constant PRFI_WEIGHT = 800000000000000000;

	/// @notice WETH weight
	uint256 public constant WETH_WEIGHT = 200000000000000000;

	/// @notice Initial swap fee percentage
	uint256 public constant INITIAL_SWAP_FEE_PERCENTAGE = 1000000000000000;

	/// @notice In 80/20 pool, PRFI Weight is 4x of WETH weight
	...
	function initialize(
		...
		require(_inTokenAddr != address(0), AddressZero());
		require(_outTokenAddr != address(0), AddressZero());
		require(_wethAddr != address(0), AddressZero());
		require(_vault != address(0), AddressZero());
		require(address(_poolFactory) != address(0), AddressZero());
	...
	function initializePool(string calldata _tokenName, string calldata _tokenSymbol) public onlyOwner {
		require(lpTokenAddr == address(0), PoolExists());
		...
		if (token0 == outTokenAddr) {
			weights[0] = PRFI_WEIGHT;
			weights[1] = WETH_WEIGHT;
		} else {
			weights[0] = WETH_WEIGHT;
			weights[1] = PRFI_WEIGHT;
		}
	 ...
	 * @param prfiPriceInEth PRFI price in ETH
	...
	function getLpPrice(uint256 prfiPriceInEth) public view returns (uint256 priceInEth) {
		...
		(uint256 prfiBalance, uint256 wethBalance, ) = getReserves();
		...
		if (token0 == outTokenAddr) {
			prfiWeight = weights[0];
			wethWeight = weights[1];
		} else {
			prfiWeight = weights[1];
			wethWeight = weights[0];
		}

		// PRFI in eth, 8 decis
		uint256 pxA = prfiPriceInEth;
		// ETH in eth, 8 decis
	...
	/**
	 * @notice Returns PRFI price in WETH
	 * @return PRFI price
	 */
	function getPrice() public view returns (uint256) {
		address vaultAddress = vaultAddr;
		/// @dev Ensure not in vault context
		...
		uint256 prfiBalance = address(tokens[0]) == outTokenAddr ? balances[0] : balances[1];
		uint256 wethBalance = address(tokens[0]) == outTokenAddr ? balances[1] : balances[0];

		return (wethBalance * 1e8) / (prfiBalance / POOL_WEIGHT);
	}
	...
	function getReserves() public view returns (uint256 prfi, uint256 weth, uint256 lpTokenSupply) {
		...
		prfi = address(tokens[0]) == outTokenAddr ? balances[0] : balances[1];
	 ...
	 * @param _prfiAmt PRFI amount
	 * @return liquidity amount of LP token
	 */
	function _joinPool(uint256 _wethAmt, uint256 _prfiAmt) internal returns (uint256 liquidity) {
		...
		if (token0 == inTokenAddr) {
			...
			maxAmountsIn[1] = _prfiAmt;
		} else {
			maxAmountsIn[0] = _prfiAmt;
	...
	function flikWETH(uint256 amount) public returns (uint256 liquidity) {
		require(msg.sender == flik, InsufficientPermission());
	...
	function flikTokens(uint256 _wethAmt, uint256 _prfiAmt) public returns (uint256 liquidity) {
		...
		IERC20(outTokenAddr).safeTransferFrom(msg.sender, address(this), _prfiAmt);

		liquidity = _joinPool(_wethAmt, _prfiAmt);
		IERC20 lp = IERC20(lpTokenAddr);
		lp.safeTransfer(msg.sender, liquidity);
	...
	function swapToWeth(address _inToken, uint256 _amount, uint256 _minAmountOut) external {
		if (msg.sender != flik) revert InsufficientPermission();
	...
	function _swap(
		...
		/// @dev Approve if allowance is less than amount
