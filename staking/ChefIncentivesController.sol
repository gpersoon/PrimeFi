...
import {ILooper} from "../../interfaces/ILooper.sol";
...
contract ChefIncentivesController is Initializable, PausableUpgradeable, OwnableUpgradeable, RecoverERC20 {
	...
	/// @dev Emitted when reward schedule is updated
	event EmissionScheduleAppended(uint256[] startTimeOffsets, uint256[] rewardsPerSeconds);

	/// @notice Emitted when the reward reserve is low
	event ChefReserveLow(uint256 indexed _balance);

	/// @notice Emitted when a user is disqualified
	...
	/// @notice Emitted when eligibility is enabled/disabled
	event EligibilityEnabledUpdated(bool indexed _newVal);

	/// @notice Emitted when a batch of alloc points are updated
	event BatchAllocPointsUpdated(address[] _tokens, uint256[] _allocPoints);

	/// @notice Emitted when looper is updated
	event LooperUpdated(ILooper _looper);

	/// @notice Emitted when ending time is updated
	event EndingTimeUpdateCadence(uint256 indexed _lapse);

	/// @notice Emitted when a reward deposit is made
	...
	/// @notice multiplier for reward calc
	uint256 private constant ACC_REWARD_PRECISION = 1e12;

	/// @notice Data about the future reward rates. emissionSchedule stored in chronological order,
	// whenever the duration since the start timestamp exceeds the next timestamp offset a new
	// reward rate is applied.
	EmissionPoint[] public emissionSchedule;

	/// @notice If true, keep this new reward rate indefinitely
	...
	/// @notice last RPS, used during refill after reserve empty
	uint256 public lastRPS;

	/// @notice Index in emission schedule which the last rewardsPerSeconds was used
	...
	/// @notice Mapping of valid reward tokens
	...
	function initialize(
		...
		require(_poolConfigurator != address(0) && address(_eligibleDataProvider) != address(0) && address(_rewardMinter) != address(0), AddressZero());
	...
	/**
	 * @dev Starts PRFI emission.
	 */
	function start() public onlyOwner {
		require(startTime == 0, AlreadyStarted());
	...
	function addPool(address _token, uint256 _allocPoint) external {
		...
		require(poolInfo[_token].lastRewardTime == 0, PoolExists());
		_updateEmissions();
	...
	function batchUpdateAllocPoint(address[] calldata _tokens, uint256[] calldata _allocPoints) external onlyOwner {
		require(_tokens.length == _allocPoints.length, ArrayLengthMismatch());
		_massUpdatePools();
	...
	function setScheduledRewardsPerSecond() internal {
		...
		/// @dev If persist is true, then RPS is fixed
	...
	function _checkDuplicateSchedule(uint256 _startTimeOffset) internal view returns (bool) {
		...
		/// @dev Check if the specified time offset is already registered
		for (uint256 i; i < length; ) {
	...
	/**
	 * @notice Updates PRFI emission schedule.
	...
	function setEmissionSchedule(
		...
		require(length > 0 && length == _rewardsPerSecond.length, ArrayLengthMismatch());

		/// @dev Append new emission schedule
		for (uint256 i; i < length; ) {
			if (i > 0) {
				require(_startTimeOffsets[i - 1] <= _startTimeOffsets[i], NotAscending());
			}
			require(_startTimeOffsets[i] <= type(uint128).max, ExceedsMaxInt());
			require(_rewardsPerSecond[i] <= type(uint128).max, ExceedsMaxInt());
			require(!_checkDuplicateSchedule(_startTimeOffsets[i]), DuplicateSchedule());

			if (startTime > 0) {
				require(_startTimeOffsets[i] >= block.timestamp - startTime, InvalidStart());
			}
	...
	function _updateEmissions() internal {
		...
		/// @dev If actual time is greater than the end time, then update all pools
	...
	function pendingRewards(address _user, address[] memory _tokens) public view returns (uint256[] memory) {
		...
		/// @dev Calculate claimable rewards for each token
		for (uint256 i; i < length; ) {
	...
	function claim(address _user, address[] memory _tokens) public whenNotPaused {
		...
		/// @dev Claim rewards for each token
		for (uint256 i; i < length; ) {
			require(validRTokens[_tokens[i]], InvalidRToken());
			PoolInfo storage pool = poolInfo[_tokens[i]];
			require(pool.lastRewardTime != 0, UnknownPool());
		...
		if (endRewardTime() < currentTimestamp + 5 days) {
			...
			emit ChefReserveLow(IERC20(prfiToken).balanceOf(address(this)));
		}
	}
	 ...
	 * @dev Can be called by owner or looper contract
	...
	function setEligibilityExempt(address _contract, bool _value) public {
		require(msg.sender == owner() || msg.sender == address(looper), InsufficientPermission());
		eligibilityExempt[_contract] = _value;
	}

	/**
	 * @notice Updates looper, only callable by owner.
	 * @param _looper contract
	 */
	function setLooper(ILooper _looper) external onlyOwner {
		looper = _looper;
		emit LooperUpdated(_looper);
	}
	...
	function handleActionAfter(address _user, uint256 _balance, uint256 _totalSupply) external {
		require(validRTokens[msg.sender] || msg.sender == address(_getMfd()), NotRTokenOrMfd());
	...
	/**
	 * @dev Stop PRFI emissions for specific users
	 * @param _user address of recipient
	 */
	function stopEmissionsFor(address _user) internal {
		require(eligibilityEnabled, NotEligible());
	...
	function setEndingTimeUpdateCadence(uint256 _lapse) external onlyOwner {
		require(_lapse <= 1 weeks, CadenceTooLong());
	...
	/**
	 * @notice Sum of all pending PRFI rewards.
	 * @param _user address of the user
