	...
	RecoverERC20
{
	...
	function initialize(
		...
		require(
			prfiToken_ != address(0) &&
			flik_ != address(0) &&
			dao_ != address(0) &&
			priceProvider_ != address(0),
			AddressZero()
		);
		require(
			rewardsDuration_ > 0 && rewardsLookback_ > 0 && lockDuration_ > 0 && vestDuration_ > 0,
			AmountZero()
		);
	...
	function earnedBalances(
		...
		for (uint256 i; i < length; ) {
			...
			if (earning.unlockTime > currentTimestamp) {
				...
				(, uint256 penaltyAmount, , ) = _ieeWithdrawableBalance(user, earning.unlockTime);
