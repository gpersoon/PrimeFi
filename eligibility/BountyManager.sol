...
contract BountyManager is Initializable, OwnableUpgradeable, PausableUpgradeable, RecoverERC20 {
	...
	/// @notice MFD token address
	address public mfd;

	/// @notice CIC token address
	address public chef;

	/// @notice PriceProvider service address
	address public priceProvider;

	/// @notice Eligibility data provider address
	address public eligibilityDataProvider;

	/// @notice Compounder address
	address public compounder;

	/// @notice % of reclaimed rewards to send to Hunter
	uint256 public hunterShare;

	/// @notice Base Bounty is paid in PRFI, will scale to match this USD target value
	uint256 public baseBountyUsdTarget; // decimals 18

	/// @notice Cap the scaling above
	uint256 public maxBaseBounty;

	/// @notice Number of bounties available
	...
	modifier isWhitelisted() {
		if (whitelistActive) {
			require(whitelist[msg.sender] || msg.sender == address(this), NotWhitelisted());
		}
		_;
	}

	/// @notice Emit when minStakeAmount is updated
	event MinStakeAmountUpdated(uint256 indexed _minStakeAmount);

	/// @notice Emit when baseBountyUsdTarget is updated
	event BaseBountyUsdTargetUpdated(uint256 indexed _newVal);
	
	/// @notice Emit when hunterShare is updated
	event HunterShareUpdated(uint256 indexed _newVal);

	/// @notice Emit when maxBaseBounty is updated
	event MaxBaseBountyUpdated(uint256 indexed _newVal);

	/// @notice Emit when bounties are set
	event BountiesSet();

	/// @notice Emit when bounty reserve is empty
	event BountyReserveEmpty(uint256 indexed _bal);

	/// @notice Emit when whitelist is updated
	event WhitelistUpdated(address indexed _user, bool indexed _isActive);

	/// @notice Emit when whitelist active status is changed
	event WhitelistActiveChanged(bool indexed isActive);

	/// @notice Error throw when address is zero
	error AddressZero();

	/// @notice Error throw when theres an invalid number set
	error InvalidNumber();

	/// @notice Error throw when the quote fails
	error QuoteFail();

	/// @notice Error throw when user is ineligible
	error Ineligible();

	/// @notice Error throw when is returned a invalid slippage
	error InvalidSlippage();

	/// @notice Error throw when action type index is out of bounds
	error ActionTypeIndexOutOfBounds();

	/// @notice Error throw when user is not whitelisted
	 ...
	 * @param _mfd MFD, to send bounties as vesting PRFI to Hunter (user calling bounty)
	 * @param _chef CIC, to query bounties for ineligible emissions
	 * @param _priceProvider PriceProvider service, to get PRFI price for bounty quotes
	 ...
	 * @param _baseBountyUsdTarget Base Bounty is paid in PRFI, will scale to match this USD target value
	...
	function initialize(
		...
		require(
			...
			_eligibilityDataProvider != address(0) &&
			_compounder != address(0),
			AddressZero()
		);

		require(
			_hunterShare <= RATIO_DIVISOR &&
			_baseBountyUsdTarget != 0 &&
			_maxBaseBounty != 0,
			InvalidNumber()
		);
	 ...
	 * @param _user address
	 * @return bounty amount of PRFI Hunter will recieve.
	 ...
	 * @return bounty in PRFI to be paid to Hunter (via vesting)
	...
	function executeBounty(
		...
		if (_execute && msg.sender != address(this)) {
			require(_canBountyHunt(msg.sender), Ineligible());
		}
		uint256 totalBounty;
		bool issueBaseBounty;
		address incentivizer;

		/// @dev Get the most appropriate bounty to run, and
		/// @dev return the amount of PRFI to be paid to Hunter
		...
		/// @dev Send the bounty to the Hunter
		if (_execute && bounty != 0) {
			if (!issueBaseBounty) {
				IERC20(prfi).safeTransferFrom(incentivizer, address(this), totalBounty);
			}
	 ...
	 * @return totalBounty raw amount of PRFI returned from Incentivizer. Hunter % will be deducted from this.
	 * @return issueBaseBounty whether Incentivizer will pay bounty from its own PRFI reserve, or from this contracts PRFI reserve
	...
	function _getAvailableBounty(
		...
		require(_actionTypeIndex <= bountyCount, ActionTypeIndexOutOfBounds());

		/// @dev If actionType is not 0, execute that bounty, else iterate over all bounties
	 ...
	 * @return totalBounty PRFI to pay for this _user's bounty execution
	...
	function _getMfdBounty(
		...
		/// @dev call multifeedistribution contract to claim bounty
	...
	function _getChefBounty(
		...
		/// @dev call chef incentives controller contract to claim bounty
	 ...
	 * @return totalBounty PRFI to pay for this _user's bounty execution. paid from Autocompound fee
	...
	function _getAutoCompoundBounty(
		...
		/// @dev call compounder contract to claim bounty
	...
	function _sendBounty(address _to, uint256 _amount) internal returns (uint256) {
		uint256 bountyReserve = IERC20(prfi).balanceOf(address(this));
		/// @dev if bounty reserve is empty, pause the contract
		if (_amount > bountyReserve) {
			IERC20(prfi).safeTransfer(address(mfd), bountyReserve);
			IMFDPlus(mfd).vestTokens(_to, bountyReserve, true);
			emit BountyReserveEmpty(bountyReserve);
			_pause();
			return bountyReserve;
		} else {
			IERC20(prfi).safeTransfer(address(mfd), _amount);
	...
	/**
	 * @notice Return PRFI amount for Base Bounty.
	 * Base Bounty used to incentivize operations that don't generate their own reward to pay to Hunter.
	 * @return bounty in PRFI
	 */
	function getBaseBounty() public view whenNotPaused returns (uint256) {
		uint256 prfiPrice = IPriceProvider(priceProvider).getTokenPriceUsd();
		uint256 bounty = (baseBountyUsdTarget * 1e8) / prfiPrice;
	...
	function setHunterShare(uint256 _newVal) external onlyOwner {
		require(_newVal <= RATIO_DIVISOR, InvalidNumber());
		hunterShare = _newVal;
