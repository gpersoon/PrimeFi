...
contract EligibilityDataProvider is OwnableUpgradeable {
	...
	/// @notice Error throw when LP token is already set
	error LPTokenSet();

	/// @notice Error throw when ratio is invalid
	error InvalidRatio();

	/// @notice Error throw when an address is not CIC
	error OnlyCIC();
	...
	function initialize(
		...
		require(
			...
			address(_priceProvider) != address(0),
			AddressZero()
		);
	...
	function setChefIncentivesController(IChefIncentivesController _chef) external onlyOwner {
		require(address(_chef) != address(0), AddressZero());
		chef = _chef;
	...
	function setLPToken(address _lpToken) external onlyOwner {
		require(_lpToken != address(0), AddressZero());
		require(lpToken == address(0), LPTokenSet());
		lpToken = _lpToken;
	...
	function setRequiredDepositRatio(uint256 _requiredDepositRatio) external onlyOwner {
		require(_requiredDepositRatio <= RATIO_DIVISOR, InvalidRatio());
	...
	function setPriceToleranceRatio(uint256 _priceToleranceRatio) external onlyOwner {
		require(
			_priceToleranceRatio >= MIN_PRICE_TOLERANCE_RATIO && _priceToleranceRatio <= RATIO_DIVISOR,
			InvalidRatio()
		);
	...
	function setDqTime(address _user, uint256 _time) external {
		require(msg.sender == address(chef), OnlyCIC());
	...
	/**
	 * @notice Returns locked PRFI and LP token value in eth
	...
	function requiredUsdValue(address user) public view returns (uint256 required) {
		/// @dev This function is used to calculate the required USD value to be locked
	...
	/**
	 * @notice Returns locked PRFI and LP token value in USD
