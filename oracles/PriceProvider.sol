...
contract PriceProvider is Initializable, OwnableUpgradeable {
	...
	function initialize(IChainlinkAdapter _baseAssetChainlinkAdapter, IPoolHelper _poolHelper) public initializer {
		require(address(_baseAssetChainlinkAdapter) != address(0), AddressZero());
		require(address(_poolHelper) != address(0), AddressZero());
	...
	function getLpTokenPrice() public view returns (uint256) {
		// decis 8
		uint256 prfiPriceInEth = getTokenPrice();
		return poolHelper.getLpPrice(prfiPriceInEth);
	}
	...
	function setOracle(address _newOracle) external onlyOwner {
		require(_newOracle != address(0), AddressZero());
