...
contract UniV3TwapOracle is BaseOracle {
	 ...
	 * @param _prfi PRFI token address.
	...
	function initialize(
		...
		require(_pair != address(0), AddressZero());
		require(_prfi != address(0), AddressZero());
		require(_ethChainlinkFeed != address(0), AddressZero());
		require(_lookbackSecs != 0, InvalidLoopbackSecs());
		...
		priceInToken0 = false;
		__BaseOracle_init(_prfi, _ethChainlinkFeed);
	}
	...
	function setTWAPLookbackSec(uint32 _secs) external onlyOwner {
		require(_secs != 0, InvalidLoopbackSecs());
		lookbackSecs = _secs;
